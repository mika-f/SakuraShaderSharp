...
namespace NatsunekoLaboratory.SakuraShader.ScreenFX.Shader.Fragment
{
    ...
    internal static class DistortionEffects
    {
        ...
        public static void ApplyScreenRotation(ref NormalizedUV uv)
        {
            ...
            uv = offsetRoll + Utilities.RotateByAngle(new NormalizedUV(uv.X * ratio, uv.Y) - offsetRoll, Radians(GlobalProperties.ScreenRotationRoll));
            uv.X /= ratio;
        }
        ...
        {
            var rotate = Utilities.RotateByAngle(new NormalizedUV(uv.X * Utilities.GetAspectRatio(), uv.Y), Radians(GlobalProperties.CheckerboardAngle));
            var cols = Floor(rotate.X * (100 - GlobalProperties.CheckerboardWidth * 100));
            var rows = Floor(rotate.Y * (100 - GlobalProperties.CheckerboardHeight * 100));

            var offset = new NormalizedUV(1 / (100 - GlobalProperties.CheckerboardWidth * 100), 1 / (100 - GlobalProperties.CheckerboardHeight * 100));

            uv.X = Lerp(uv.X, uv.X + offset.X * GlobalProperties.CheckerboardOffset, Utilities.IsEquals(Fmod(cols + rows, 2), 0));
        }
    }
}
#endif